...
//           .then(() => console.log('nextPrayer stored'))
...
//       .then(() => console.log('upcomingPrayer stored'))
//       .catch(error => console.error('Failed to store upcomingPrayer', error));
//   }

//   console.log('Next prayer:', currentPrayer);
//   console.log('Next prayer:', upcomingPrayer);
...
const PrayerTimes = ({
    ...
    try {
      ...
      const prayers = [
        ...
        },
        {
          ...
          time: format(prayerTimes.sunrise, 'h:mm a'),
          isCurrentPrayer: false,
          originalTime: prayerTimes.sunrise,
        },
        {
          name: 'Dhuhr',
          arabicName: PRAYER_ARABIC_NAMES.dhuhr,
          time: format(prayerTimes.dhuhr, 'h:mm a'),
          isCurrentPrayer: false,
          originalTime: prayerTimes.dhuhr,
        },
        {
          name: 'Asr',
          arabicName: PRAYER_ARABIC_NAMES.asr,
        ...
        },
        {
          ...
          time: format(prayerTimes.maghrib, 'h:mm a'),
          isCurrentPrayer: false,
          originalTime: prayerTimes.maghrib,
        },
        {
          name: 'Isha',
          arabicName: PRAYER_ARABIC_NAMES.isha,
      ...
      prayers.forEach(prayer => {
        ...
        if (nextPrayerName === lowerName) {
          prayer.isCurrentPrayer = true;
          prayer.remainingTime = getFormattedRemainingTime(prayer.originalTime);
          nextPrayer = {...prayer};
        }
      });

      setAllPrayers(prayers);
      setCurrentPrayer(nextPrayer || prayers[0]); // Fallback to first prayer if none is "current"
      ...
      setError(error.message || 'Failed to calculate prayer times');
  ...
  if (!currentPrayer || allPrayers.length === 0) {
    return (
      <View style={styles.errorContainer}>
        ...
        <View style={styles.compactCard}>
          ...
          <View style={styles.compactContent}>
            {upcomingPrayer ? (
              ...
              <Text style={styles.noUpcomingText}>No upcoming prayer</Text>
            )}
          </View>
        </View>
      </View>
